...
public class ViewRedirector implements Redirector {
    ...
    public ViewRedirector(String view) {
        if (view == null || view.trim().equals(""))
            throw new IllegalArgumentException(
                    "Not a valid view for redirection: " + view);
    ...
    String getFileName(String thisComponentName, String thisLogicName) {
        ...
        if (logger.isDebugEnabled())
            logger.debug("Found view redirection to: " + component + "."
                    + logic + "." + result);

        return REGEX_LOCATION.replaceFirst("\\$component", component)
                .replaceFirst("\\$logic", logic).replaceFirst("\\$result",
                        result);
    }
